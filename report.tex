...
\section{localStorage e sessionStorage} %<-----PARLO ANCHE DEL CODE SNIPPET PER VERIFICARE LA DISPONIBILITÀ?
\texttt{\textit{localStorage}} e \texttt{\textit{sessionStorage}} sono entrambi oggetti di tipo \texttt{Storage}: in quanto tali, essi permettono di salvare dati in memoria locale come coppie chiave-valore (entrambe stringhe) \cite{MDN_Web_docs:storage_API}.\\
I due oggetti differscono per visibilità e durata: \texttt{localStorage} serve a contenere dati permanenti (cioè che rimangono in memoria indefinitamente, salvo esplicita rimozione da parte dell'utente o del programmatore) e condivisi fra le varie schede, per cui schede distinte del medesimo browser connesse allo stesso sito condividono lo stesso \texttt{localStorage} (modifiche apportate da una pagina sono visibili anche nell'altra pagina). D'altro canto \texttt{sessionStorage} è progettato per contenere dati relativi alla singola sessione di navigazione: pertanto esso non è condiviso fra schede e viene rimosso automaticamente alla chiusura della pagina (è però in grado di sopravvivere ad aggiornamenti della pagina stessa).\\
\subsection{implementazione}
...
\begin{itemize}
\item\texttt{setItem(key, value)}, che inserisce una nuova coppia chiave-valore in \texttt{Storage}. Se la chiave è già presente allora sostituisce il precedente valore a essa associato con il nuovo. \texttt{key} e \texttt{value} devono essere entrambi stringhe.
\item\texttt{getItem(key)}, che restituisce il valore associato a \texttt{key}.
\item\texttt{removeItem(key)}, che rimuove la coppia con chiave \texttt{key}.
\item\texttt{clear()}, che svuota l'intero \texttt{Storage}.
\item\texttt{key(n)}, che restituisce il nome della \texttt{n}-esima chiave. 
\end{itemize}
